...
export default function FinalizarConvitePlataformaClient({ token }: Props) {
  ...
  async function finalizarCadastro() {
    ...
    if (!acceptResponse.ok || !acceptPayload?.success) {
      setEtapa("idle");
      setErro(acceptPayload?.error ?? "Não foi possível aceitar o convite da plataforma.");
      return;
    }
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#06080f] p-4 md:p-6">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(217,70,239,0.16),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(99,102,241,0.18),transparent_35%)]" />
      ...
      <section className="relative z-10 w-full max-w-md rounded-[28px] border border-white/15 bg-[#0d1426]/90 p-6 shadow-[0_0_80px_rgba(15,23,42,0.55)] backdrop-blur md:p-8">
        ...
        <p className="mt-2 text-sm text-slate-300">
          Preencha seus dados para entrar no ecossistema ADSYNC3.
        </p>
        ...
        <div className="mt-6 space-y-4">
          ...
          <button
            ...
            className="w-full rounded-2xl bg-gradient-to-r from-fuchsia-500 to-indigo-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-fuchsia-500/20 transition hover:brightness-110 disabled:opacity-60"
          >
